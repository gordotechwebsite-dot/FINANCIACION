...
export default function TradeInForm({ initialData, onNext, onBack }: TradeInFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="mb-4">
        <h2 className="text-lg font-bold text-primary">Equipo del Cliente</h2>
        <p className="text-gray-400 text-xs mt-0.5">Datos del equipo que entrega</p>
      </div>
      ...
      <div>
        ...
        />
        {errors.imei && <p className="text-highlight text-[11px] mt-1">{errors.imei}</p>}
      </div>
      ...
      <div>
        <label className="text-xs font-medium text-gray-500 mb-1 block">Valor aceptado (COP)</label>
        <input
          type="number"
        ...
        />
        {errors.acceptedValue && <p className="text-highlight text-[11px] mt-1">{errors.acceptedValue}</p>}
      </div>
